...
export function VideoRecap({ onComplete, stats }: VideoRecapProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center overflow-hidden bg-[var(--background)] text-[var(--foreground)]">
      <button
        type="button"
        onClick={() => setIsPaused((prev) => !prev)}
        aria-label={isPaused ? t("recap.resume") : t("recap.pause")}
        className="absolute left-6 top-6 z-50 flex items-center gap-2 text-[rgba(var(--foreground-rgb),0.6)] transition-colors hover:text-[var(--foreground)]"
      >
      ...
      <button
        type="button"
        onClick={onComplete}
        className="absolute right-6 top-6 z-50 flex items-center gap-2 text-[rgba(var(--foreground-rgb),0.6)] transition-colors hover:text-[var(--foreground)]"
      >
      ...
      <AnimatePresence mode="wait">
        {currentSceneIndex === 0 && <FirstScene stats={stats} isPaused={isPaused} />}
        {currentSceneIndex === 1 && <SecondScene stats={stats} isPaused={isPaused} />}
        {currentSceneIndex === 2 && <ThirdScene stats={stats} isPaused={isPaused} />}
        {currentSceneIndex === 3 && <FourthScene stats={stats} isPaused={isPaused} />}
        {currentSceneIndex === 4 && <FifthScene stats={stats} isPaused={isPaused} />}
      </AnimatePresence>
    </div>
  );
}
